...
/// ShapleAuth
class ShapleAuth with WidgetsBindingObserver {
  ...
  Future<void> initialize({
    ...
    _authSubscription = Shaple.instance.client.auth.onAuthStateChange.listen(
      (data) {
        _onAuthStateChange(data.event, data.session);
      },
      onError: (error, stackTrace) {
        Shaple.instance.log(error.toString(), stackTrace);
      },
    );
    ...
    if (hasPersistedSession) {
      ...
      if (persistedSession != null) {
        try {
          Shaple.instance.client.auth.setInitialSession(persistedSession);
  ...
  ///
  /// Called lazily after `.initialize()` by `Shaple` instance
  Future<void> recoverSession() async {
    try {
      ...
      if (hasPersistedSession) {
        final persistedSession = await _localStorage.accessToken();
        if (persistedSession != null) {
          await Shaple.instance.client.auth.recoverSession(persistedSession);
        }
      }
    } on AuthException catch (error, stackTrace) {
      Shaple.instance.log(error.message, stackTrace);
    } catch (error, stackTrace) {
      Shaple.instance.log(error.toString(), stackTrace);
    }
  }
  ...
  Future<void> _recoverShapleSession() async {
    ...
    try {
      await Shaple.instance.client.auth.recoverSession(jsonStr);
    } catch (error) {
  ...
  void _onAuthStateChange(AuthChangeEvent event, Session? session) {
    Shaple.instance.log('**** onAuthStateChange: $event');
    if (session != null) {
      Shaple.instance.log(jsonEncode(session.toJson()));
  ...
  Future<void> _startDeeplinkObserver() async {
    Shaple.instance.log('***** ShapleDeepLinkingMixin startAuthObserver');
  ...
  void _stopDeeplinkObserver() {
    Shaple.instance.log('***** ShapleDeepLinkingMixin stopAuthObserver');
  ...
  Future<void> _handleDeeplink(Uri uri) async {
    ...
    Shaple.instance.log('***** ShapleAuthState handleDeeplink $uri');

    // notify auth deeplink received
    Shaple.instance.log('onReceivedAuthDeeplink uri: $uri');

    try {
      await Shaple.instance.client.auth.getSessionFromUrl(uri);
    } on AuthException catch (error, stackTrace) {
      // ignore: invalid_use_of_internal_member
      Shaple.instance.client.auth.notifyException(error, stackTrace);
      Shaple.instance.log(error.toString(), stackTrace);
    } catch (error, stackTrace) {
      Shaple.instance.log(error.toString(), stackTrace);
    }
...
extension GoTrueClientSignInProvider on GoTrueClient {
  ...
  ///
  /// ```dart
  /// await shaple.auth.signInWithOAuth(
